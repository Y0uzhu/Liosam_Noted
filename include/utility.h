...
public:
    ...
    // imu2lidar的外参，这里有两个旋转外参extRot和extRPY（extQRPY是extRPY的逆），
    // 是因为在作者使用的IMU中，旋转的坐标系和其余的不一样，所以要分开使用
    ...
    /**
     * @brief 把IMU的信息，从IMU坐标系，转换到雷达坐标系。（注意，这个函数只旋转，没有平移，和真正的雷达坐标系之间还是差了一个平移的。)
     * 至于为什么没有平移，因为，在imuPreintegration.cpp文件中，还有两个imu2Lidar，lidar2imu变量，这俩变量只有平移，没有旋转。
     * 实际上作者相当于是建立了一个原点与imu系相同，但姿态却与雷达坐标系相同的中间系。
     * 先把imu的数据用imuConverter旋转到中间系下，然后把雷达数据根据lidar2Imu反向平移到了中间系下。在中间系进行对齐。
     * 最后再把结果通过imu2Lidar挪回了雷达系进行publish。
     * 
     * @param imu_in 
